...
#define MESSAGE_TEMPLATE "node_id:{0};packet_id:{1};timestamp:{2};payload:{3}"
#define PACKET_MISSING_ERROR_CODE -1
#define NODE_ID 3
TemperatureZero TempZero = TemperatureZero();
int data[4][3];
int packetID = 0;
//char packetToSend[64];
...
void tx(const char *str, osjobcb_t func) {
  ...
  delay(1); // Wait a bit, without this os_radio below asserts, apparently beca
  //use the state hasn't changed yet
  LMIC.dataLen = 0;
  while (*str)
...
static void tx_func (osjob_t* job) {
  double currentTemp = TempZero.readInternalTemperature();
  String currentPacket = MESSAGE_TEMPLATE;
  
  long timeSinceLastPacket = millis();
  currentPacket.replace("{0}", String(NODE_ID));
  currentPacket.replace("{1}", String(packetID));
  currentPacket.replace("{2}", String(timeSinceLastPacket));
  currentPacket.replace("{3}", String(currentTemp));
  uint8_t msgLength = currentPacket.length() + 1;
  char toSend[64];
  currentPacket.toCharArray(toSend, 64);
  tx(toSend, txdone_func);
  packetID++;
